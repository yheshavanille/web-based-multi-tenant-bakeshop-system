<div>
            ...
            <table class="w-full divide-y divide-gray-200 text-sm">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left font-medium text-gray-700">Old → New</th>
                ...
                <tbody class="divide-y divide-gray-200 bg-white">
                    ...
                    $icon = $fieldIcons[$history->field] ?? '📌';
                    $label = $fieldLabels[$history->field] ?? ucfirst(str_replace('_', ' ', $history->field));
                    $oldDisplay = $history->old_value ?? '-';
                    $newDisplay = $history->new_value ?? '-';
                    if (strlen($oldDisplay) > 30) $oldDisplay = substr($oldDisplay, 0, 30) . '...';
                    if (strlen($newDisplay) > 30) $newDisplay = substr($newDisplay, 0, 30) . '...';
                    @endphp
                    <tr class="hover:bg-gray-50 transition">
                        <td class="px-4 py-3 font-medium text-gray-800">{{ $history->product->name }}</td>
                        <td class="px-4 py-3">
                        ...
                        <td class="px-4 py-3">
                            <span class="text-xs text-gray-500 line-through mr-1">{{ $oldDisplay }}</span>
                            <span class="text-xs font-medium text-green-600">→ {{ $newDisplay }}</span>
                        </td>
                        <td class="px-4 py-3 text-gray-600">{{ $history->user->name }}</td>
                        <td class="px-4 py-3 text-gray-400 text-xs">{{ $history->created_at->diffForHumans() }}</td>
                        <td class="px-4 py-3">
                            <button wire:click="viewProductEditDetails({{ $history->id }})"
                                class="text-xs text-blue-600 hover:text-blue-800 font-medium transition">
                                View Details
                            </button>
                        </td>
                    ...
                    <table class="w-full divide-y divide-gray-200 text-sm">
                        ...
                        <tbody class="divide-y divide-gray-200 bg-white">
                            ...
                            <tr class="hover:bg-gray-50 transition">
                                ...
                                <td class="px-4 py-3">
                                    <span
                                        class="px-2 py-1 text-xs font-medium rounded-full {{ $employee->role === 'order_manager' ? 'bg-blue-100 text-blue-800' : 'bg-purple-100 text-purple-800' }}">
                                        {{ $employee->role_label }}
                                    </span>
                                </td>
                                ...
                                <td class="px-4 py-3">
                                    <span
                                        class="px-2 py-1 text-xs font-medium rounded-full {{ $employee->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                        {{ $employee->is_active ? '🟢 Active' : '🔴 Inactive' }}
                                    </span>
                                </td>
                            </tr>
    ...
    @if($showProductEditModal && $selectedProductEdit)
    <div class="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto p-4"
        style="overscroll-behavior: contain;">
        <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" wire:click="closeProductEditModal">
        </div>

        <div class="relative z-10 w-full max-w-2xl bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col"
            style="max-height: 90vh;">
            ...
            <div class="px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-indigo-50 flex-shrink-0">
                <div class="flex items-center justify-between">
                    <div>
                        <h3 class="text-xl font-bold text-gray-800">🔍 Change Details</h3>
                        <p class="text-sm text-gray-500">
                            {{ $selectedProductEdit->product->name }}
                            <span class="text-xs text-gray-400 ml-2">•</span>
                            <span class="text-xs text-gray-500">Changed by {{ $selectedProductEdit->user->name }}</span>
                            <span class="text-xs text-gray-400 ml-2">•</span>
                            <span class="text-xs text-gray-500">{{ $selectedProductEdit->created_at->diffForHumans()
                                }}</span>
                        </p>
                    </div>
                    <button wire:click="closeProductEditModal" class="text-gray-400 hover:text-gray-600 transition">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M6 18L18 6M6 6l12 12"></path>
                        </svg>
            ...
            <div class="flex-1 overflow-y-auto px-6 py-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="bg-red-50 rounded-xl p-4 border border-red-200">
                        <h4 class="text-sm font-semibold text-red-700 mb-2">⬅️ Before</h4>
                        @if($selectedProductEdit->field === 'image_url')
                        @if($selectedProductEdit->old_value)
                        <img src="{{ asset($selectedProductEdit->old_value) }}"
                        ...
                        @endif
                        @else
                        <p class="text-sm text-gray-700">{{ $selectedProductEdit->old_value ?? '-' }}</p>
                        @endif
                    </div>

                    <!-- After -->
                    <div class="bg-green-50 rounded-xl p-4 border border-green-200">
                        <h4 class="text-sm font-semibold text-green-700 mb-2">➡️ After</h4>
                        @if($selectedProductEdit->field === 'image_url')
                        @if($selectedProductEdit->new_value)
                        <img src="{{ asset($selectedProductEdit->new_value) }}"
                            class="w-full h-40 object-cover rounded-lg">
                        @else
                        <div class="w-full h-40 bg-gray-200 rounded-lg flex items-center justify-center text-gray-400">
                            No image
                        ...
                        @endif
                        @else
                        <p class="text-sm text-gray-700">{{ $selectedProductEdit->new_value ?? '-' }}</p>
                        @endif
                    </div>
                </div>

                <!-- Enhanced Field Info with Discount Context -->
                <div class="mt-4 p-3 bg-yellow-50 rounded-lg border border-yellow-200">
                    <p class="text-xs text-gray-600">
                        💡 <span class="font-medium">Note:</span>
                        @if($selectedProductEdit->field === 'price')
                        Price changed from <span class="font-medium">₱{{ number_format($selectedProductEdit->old_value,
                            2) }}</span>
                        to <span class="font-medium">₱{{ number_format($selectedProductEdit->new_value, 2) }}</span>
                        @if($selectedProductEdit->product->isDiscounted())
                        <br>💰 Customers will pay <span class="font-bold text-green-600">₱{{
                            number_format($selectedProductEdit->product->getDiscountedPrice(), 2) }}</span>
                        (after <span class="font-bold text-red-600">{{ $selectedProductEdit->product->getDiscountLabel()
                            }}</span> discount)
                        @endif
                        @elseif($selectedProductEdit->field === 'discount_type' || $selectedProductEdit->field ===
                        'discount_value')
                        Discount changed from <span class="font-medium">{{ $selectedProductEdit->old_value ?? 'None'
                            }}</span>
                        to <span class="font-medium">{{ $selectedProductEdit->new_value ?? 'None' }}</span>
                        @elseif($selectedProductEdit->field === 'stock')
                        Stock changed from <span class="font-medium">{{ $selectedProductEdit->old_value ?? '0' }}</span>
                        to <span class="font-medium">{{ $selectedProductEdit->new_value ?? '0' }}</span>
                        @else
                        Changed from <span class="font-medium">{{ $selectedProductEdit->old_value ?? '-' }}</span>
                        to <span class="font-medium">{{ $selectedProductEdit->new_value ?? '-' }}</span>
                        @endif
                    </p>
                </div>
                ...
                <div class="mt-2 p-3 bg-gray-50 rounded-lg border border-gray-200">
                    <p class="text-xs text-gray-500">Field changed: <span class="font-medium text-gray-700">{{
                            ucfirst(str_replace('_', ' ', $selectedProductEdit->field)) }}</span></p>
                </div>
            </div>

            <!-- Modal Footer -->
            <div class="px-6 py-3 border-t border-gray-200 bg-gray-50 flex-shrink-0 flex justify-end">
                <button wire:click="closeProductEditModal"
                    class="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition text-sm font-medium">
                    Close
                </button>
            </div>
    ...
    <!-- View All Stock History Modal -->
    @if($showStockHistoryModal)
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
        <div class="fixed inset-0 transition-opacity bg-gray-500 bg-opacity-75" wire:click="closeStockHistoryModal">
        </div>

        <div class="relative w-full max-w-6xl overflow-hidden text-left transition-all transform bg-white rounded-2xl shadow-2xl flex flex-col"
            style="max-height: 90vh;">
            ...
            <div class="px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-indigo-50 flex-shrink-0">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-sm text-gray-500">{{ $allStockHistories->count() }} updates found</p>
                    </div>
                    <button wire:click="closeStockHistoryModal" class="text-gray-400 hover:text-gray-600">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M6 18L18 6M6 6l12 12"></path>
                        </svg>
            ...
            <div class="px-6 py-4 overflow-y-auto flex-1" style="max-height: 60vh;">
                    ...
                    <table class="w-full divide-y divide-gray-200 text-sm">
                        ...
                        <tbody class="divide-y divide-gray-200 bg-white">
                            ...
                            <tr>
                                ...
                                <td class="px-4 py-3 text-gray-600">{{ $history->user->name }}</td>
                                <td class="px-4 py-3 text-gray-500">{{ $history->notes ?? '-' }}</td>
                                <td class="px-4 py-3 text-gray-400 text-xs">{{ $history->created_at->diffForHumans() }}
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
                ...
                @else
                <p class="text-sm text-gray-500">No stock updates found.</p>
                @endif
            </div>
    ...
    <!-- View All Product History Modal -->
    @if($showProductHistoryModal)
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
        <div class="fixed inset-0 transition-opacity bg-gray-500 bg-opacity-75" wire:click="closeProductHistoryModal">
        </div>
        ...
        <div class="relative w-full max-w-6xl overflow-hidden text-left transition-all transform bg-white rounded-2xl shadow-2xl flex flex-col"
            ...
            <div class="px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-indigo-50 flex-shrink-0">
                <div class="flex items-center justify-between">
                    <div>
                        <h3 class="text-xl font-bold text-gray-800">📝 All Product Updates</h3>
                        <p class="text-sm text-gray-500">{{ $allProductHistories->count() }} updates found</p>
                    </div>
                    <button wire:click="closeProductHistoryModal" class="text-gray-400 hover:text-gray-600">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M6 18L18 6M6 6l12 12"></path>
                        </svg>
            ...
            <!-- Modal Body -->
            <div class="px-6 py-4 overflow-y-auto flex-1" style="max-height: 60vh;">
                @if($allProductHistories->count() > 0)
                <div class="overflow-x-auto">
                    <table class="w-full divide-y divide-gray-200 text-sm">
                        <thead class="bg-gray-50 sticky top-0">
                            <tr>
                        ...
                        <tbody class="divide-y divide-gray-200 bg-white">
                            ...
                            <tr>
                                <td class="px-4 py-3 font-medium text-gray-800">{{ $history->product->name }}</td>
                                <td class="px-4 py-3">{{ $icon }} {{ $label }}</td>
                                <td class="px-4 py-3">
                                    <span class="text-xs text-gray-500 line-through mr-1">{{ $oldDisplay }}</span>
                                    <span class="text-xs font-medium text-green-600">→ {{ $newDisplay }}</span>
                                </td>
                ...
                @else
                <p class="text-sm text-gray-500">No product updates found.</p>
                @endif
            </div>
